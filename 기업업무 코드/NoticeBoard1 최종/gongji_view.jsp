	...
	<table>
		...
		%>
				<tr>
					...
					<td class="nine"><%=id%><input type="hidden" name="id" value="<%=id%>"></td> <%-- 번호 출력 --%>
				</tr>
				<tr>
					<td class="one" style="background-color : skyblue;">제목</td> <%-- 제목 --%>
					<td class="nine"><%=title%></td> <%-- 제목 출력 --%>
				</tr>
				<tr>
				...
				<tr>
					<td class="one" style="background-color : skyblue;">내용</td> <%-- 내용 --%>
					 <%-- 내용 출력 --%>
					<td class="nine"><textarea name="updateContent" rows="20" cols="70" style="overflow: auto; border: none; resize: none;" readonly><%=content%></textarea></td>
				</tr>
		<%
			}
	...
	<table id="down">
		<tr>
			<td style="border:0px; text-align:right;">
				<input class="submit" type="submit" value="목록" formaction="gongji_list.jsp"> <%-- 목록버튼, 누르면 gongji_list에 데이터 전달 --%>
				<input class="submit" type="submit" value="수정" formaction="gongji_update.jsp"> <%-- 수정버튼, 누르면 gongji_update에 데이터 전달 --%>
			</td>
		</tr>
	</table>
</form>
...
%>
	<form method="post" action="gongji_list.jsp">
		...
		<div style="text-align:center;"><input type="submit" value="게시판으로 이동"></div> <%-- 게시판 이동 버튼 버튼 --%>
	</form>
<%
	}
