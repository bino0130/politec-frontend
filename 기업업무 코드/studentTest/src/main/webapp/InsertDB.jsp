...
<%
request.setCharacterEncoding("utf-8"); // 언어타입 utf-8로 설정
...
<form method="post" action="InputForm1.html"> <%-- 버튼을 눌렀을 때 InputForm1.html에 값을 전달하는 form태그 --%>
	...
	<table cellspacing=1 border=1px solid black> <%-- 테이블 시작 --%>
		<tr>
			<td class="one"><p>이름</p></td> <%-- 클래스명이 one인 태그 안에  --%>
			<td class="three"><p>
		...
		<tr>
			...
			<td class="three"><p>
					<input type='text' name='korean' value='<%=kor%>' readonly>
				</p></td>
		</tr>
		<tr>
			<td class="one"><p>영어</p></td>
			<td class="three"><p>
					<input type='text' name='english' value='<%=eng%>' readonly>
				</p></td>
		</tr>
		<tr>
			<td class="one"><p>수학</p></td>
			<td class="three"><p>
					<input type='text' name='math' value='<%=mat%>' readonly>
				</p></td>
		</tr>
	</table> <%-- 테이블 끝 --%>
</form>
