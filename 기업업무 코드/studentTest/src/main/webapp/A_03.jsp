...
<body>
	...
	int id = 0; // int형 정수 min 0으로 초기화
	// 비어있는 학번 확인하고 해당 값 채우거나 학번이 비어있지 않은 경우 최대값 + 1 하는 쿼리
	ResultSet settotal = stmt.executeQuery("select min(id) as minNum from kiho where (id+1) not in (select id from kiho);");
	while(settotal.next()) { // settotal이 있을떄 작동하는 반복문
		id = settotal.getInt(1) + 1; // min = getInt(1) + 1
	}
	
	String Querytxt = String.format("insert into kiho value (%d, '%s')", id, name);
