...
int beforePage = from - (cnt*10) - (currentPage-startPage)*10; // 이전 페이지 구하는 변수 beforePage
if (beforePage <= 10) { // beforePage가 10보다 작거나 같으면
    beforePage = 1; // 1로 설정
}
// <<, < 계산, 출력하는 부분
...
%>
<td><a href="wifi3.jsp?from=0&cnt=<%=cnt%>"><<</a></td> <%-- << : 1페이지로 이동 --%>
<td><a href="wifi3.jsp?from=<%=beforePage%>&cnt=<%=cnt%>"><</a></td> <%-- << : 1페이지 앞으로 이동 --%>
<%
}
